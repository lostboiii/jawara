...
class EditPemasukanPage extends StatefulWidget {
  const EditPemasukanPage({super.key, required this.pemasukan});
...
class _EditPemasukanPageState extends State<EditPemasukanPage> {
  ...
  void initState() {
    super.initState();
    _namaController = TextEditingController(text: widget.pemasukan.nama_pemasukan);
    _tanggalController = TextEditingController(
      text: DateFormat('dd/MM/yyyy').format(widget.pemasukan.tanggal_pemasukan),
    );
    _nominalController = TextEditingController(
      text: widget.pemasukan.jumlah.toInt().toString(),
    );
    _selectedKategori = widget.pemasukan.kategori_pemasukan;
    _selectedTanggal = widget.pemasukan.tanggal_pemasukan;

    if (_selectedKategori != null && !_kategoriOptions.contains(_selectedKategori)) {
      _kategoriOptions.add(_selectedKategori!);
    }

    if (widget.pemasukan.bukti_pemasukan != null) {
      _currentBuktiName = widget.pemasukan.bukti_pemasukan!.split('/').last;
    }
  }
            ...
            child: Column(
              ...
              children: [
                ...
                Text(
                  _selectedFile != null
                      ? _selectedFile!.name
                      : (_currentBuktiName ?? 'Unggah Bukti Pemasukan'),
                  style: GoogleFonts.inter(
                    fontSize: 14,
                    fontWeight: FontWeight.w500,
  ...
  Future<void> _handleSimpan() async {
    ...
    try {
      ...
      String? buktiUrl = widget.pemasukan.bukti_pemasukan;
      ...
      } else if (_currentBuktiName == null) {
        buktiUrl = null;
      }

      await viewModel.updatePemasukan(
        id: widget.pemasukan.id,
        nama_pemasukan: _namaController.text,
      ...
      await showDialog(
        ...
        builder: (BuildContext context) {
          return AlertDialog(
            ...
            content: Text(
              'Perubahan data pemasukan berhasil disimpan',
              textAlign: TextAlign.center,
              style: GoogleFonts.inter(fontSize: 14),
            ),
            actions: [
