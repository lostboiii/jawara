...
class PengeluaranEditPage extends StatefulWidget {
  const PengeluaranEditPage({super.key, required this.pengeluaran});

  final PengeluaranModel pengeluaran;

  @override
  State<PengeluaranEditPage> createState() => _PengeluaranEditPageState();
}

class _PengeluaranEditPageState extends State<PengeluaranEditPage> {
  ...
  void initState() {
    super.initState();
    _namaController =
        TextEditingController(text: widget.pengeluaran.namaPengeluaran);
    _tanggalController = TextEditingController(
      text: DateFormat('dd/MM/yyyy')
          .format(widget.pengeluaran.tanggalPengeluaran),
    );
    _nominalController = TextEditingController(
      text: widget.pengeluaran.jumlah.toInt().toString(),
    );
    _selectedKategori =
        _resolveKategoriValue(widget.pengeluaran.kategoriPengeluaran);
    _selectedTanggal = widget.pengeluaran.tanggalPengeluaran;

    if (widget.pengeluaran.buktiPengeluaran != null) {
      _currentBuktiName = widget.pengeluaran.buktiPengeluaran!.split('/').last;
    }
  }
                ...
                child: Consumer<PengeluaranViewModel>(
                  builder: (context, viewModel, child) {
                    return ElevatedButton(
                      onPressed: viewModel.isLoading ? null : _handleSimpan,
      ...
      children: [
        Row(
          ...
          children: [
            ...
            if (_selectedFile != null || _currentBuktiName != null)
              GestureDetector(
                onTap: () {
                  setState(() {
                    _selectedFile = null;
                    _currentBuktiName = null;
                  });
                },
                child: Text(
                  'Hapus',
        ...
        GestureDetector(
          ...
          child: Container(
            ...
            child: Column(
              ...
              children: [
                Icon(
                  (_selectedFile != null || _currentBuktiName != null)
                ...
                Text(
                  _selectedFile != null
                      ? _selectedFile!.name
                      : (_currentBuktiName ?? 'Unggah Bukti Pengeluaran'),
                  style: GoogleFonts.inter(
                    fontSize: 14,
                    fontWeight: FontWeight.w500,
                    color: (_selectedFile != null || _currentBuktiName != null)
                        ? Colors.black87
  ...
  Future<void> _handleSimpan() async {
    ...
    try {
      ...
      String? buktiUrl = widget.pengeluaran.buktiPengeluaran;

      if (_currentBuktiName == null) {
        buktiUrl = null;
      }
